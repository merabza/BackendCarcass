...
public static class ProcessesEndpoints
{
    ...
    {
        if (debugMode) logger.Information("{MethodName} Started", nameof(UseProcessesEndpoints));
        ...
        if (debugMode) logger.Information("{MethodName} Finished", nameof(UseProcessesEndpoints));
