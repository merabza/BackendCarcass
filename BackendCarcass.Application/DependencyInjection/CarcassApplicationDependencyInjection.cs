...
public static class CarcassApplicationDependencyInjection
{
    ...
    {
        if (debugMode) logger.Information("{MethodName} Started", nameof(AddScopedAllCarcassApplicationServices));
        ...
        if (debugMode) logger.Information("{MethodName} Finished", nameof(AddScopedAllCarcassApplicationServices));
